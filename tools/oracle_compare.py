...
Environment:
    AVL_BASIC_PY_ORACLE points directly to basic.py in the external reference repo.
...
def main() -> int:
    ...
    oracle_value = os.environ.get("AVL_BASIC_PY_ORACLE")
    if not oracle_value:
        raise SystemExit("Set AVL_BASIC_PY_ORACLE to the external reference basic.py")
    oracle = Path(oracle_value).resolve()
